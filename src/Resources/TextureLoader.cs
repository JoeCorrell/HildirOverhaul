...
namespace HildirOverhaul
{
    ...
    public static class TextureLoader
    {
        ...
        public static Texture2D LoadUITexture(string name)
        {
            ...
            {
                HildirOverhaul.Log.LogWarning("TextureLoader: ImageConversion.LoadImage not found via reflection.");
                return null;
            }

            string resourceName = $"HildirOverhaul.Resources.Textures.UI.{name}.png";
            using (Stream stream = ModAssembly.GetManifestResourceStream(resourceName))
            {
                if (stream == null)
                {
                    HildirOverhaul.Log.LogWarning($"TextureLoader: UI resource '{resourceName}' not found.");
                    return null;
                }
