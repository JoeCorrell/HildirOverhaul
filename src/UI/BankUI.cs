...
namespace HildirOverhaul
{
    public class BankUI : MonoBehaviour
    {
        ...
        private void BuildUI()
        {
            if (!ExtractAssets())
            {
                HildirOverhaul.Log.LogError("[BankUI] Failed to extract Valheim assets.");
                return;
            }

            // ── Canvas ──
            _canvasGO = new GameObject("HildirBank_Canvas");
            ...
            // ── Title ──
            _titleText = CreateText(content, "Title", "Hildir's Bank", 26f, GoldTextColor, TextAlignmentOptions.Center);
