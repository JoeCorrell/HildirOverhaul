...
namespace HildirOverhaul
{
    public class TraderUI : MonoBehaviour
    {
        ...
        // ── Right column: Hildir preview (sell tab) ──
        private RenderTexture _hildirPreviewRT;
        private GameObject _hildirCamGO;
        private Camera _hildirCam;
        private GameObject _hildirClone;
        private GameObject _hildirLightRig;
        private RawImage _hildirPreviewImg;
        private static readonly Vector3 HildirSpawnPos = new Vector3(10000f, 5000f, 10020f);
        ...
        private void Update()
        {
            ...
            // Preview rotation (player only; Hildir camera is fixed)
        ...
        private void LateUpdate()
        {
            ...
            if (hud != null)
            {
                ...
                // Suppress the "Talk to Hildir [E]" hover text while our UI is open
            ...
            try
            {
                ...
                if (_activeTab == 0 && _playerCam != null)
                    _playerCam.Render();
                else if (_activeTab == 1 && _hildirCam != null)
                    _hildirCam.Render();
            }
            finally
            {
        ...
        private void OnDestroy()
        {
            ...
            if (_hildirCamGO != null) Destroy(_hildirCamGO);
            if (_playerPreviewRT != null) { _playerPreviewRT.Release(); Destroy(_playerPreviewRT); }
            if (_hildirPreviewRT != null) { _hildirPreviewRT.Release(); Destroy(_hildirPreviewRT); }
        ...
        private void BuildUI()
        {
            if (!ExtractAssets())
            {
                HildirOverhaul.Log.LogError("[TraderUI] Failed to extract Valheim assets.");
                return;
            }

            // Canvas
            _canvasGO = new GameObject("HildirOverhaul_Canvas");
        ...
        private void BuildPreviewColumn()
        {
            ...
            // Hildir preview RT + camera
            _hildirPreviewRT = new RenderTexture(rtW, rtH, 24, RenderTextureFormat.ARGB32);
            _hildirPreviewRT.antiAliasing = 4;
            _hildirPreviewRT.filterMode = FilterMode.Trilinear;

            _hildirCamGO = new GameObject("TraderUI_HildirCam");
            DontDestroyOnLoad(_hildirCamGO);
            _hildirCam = _hildirCamGO.AddComponent<Camera>();
            _hildirCam.targetTexture = _hildirPreviewRT;
            _hildirCam.clearFlags = CameraClearFlags.SolidColor;
            _hildirCam.backgroundColor = new Color(0f, 0f, 0f, 0f);
            _hildirCam.fieldOfView = 30f;
            _hildirCam.nearClipPlane = 0.1f;
            _hildirCam.farClipPlane = 10f;
            _hildirCam.depth = -2;
            _hildirCam.enabled = false;
            _hildirCam.cullingMask = mask;
            ...
            // Hildir RawImage
            var hildirImgGO = new GameObject("HildirPreview", typeof(RectTransform));
            hildirImgGO.transform.SetParent(_rightColumn, false);
            var hRT = hildirImgGO.GetComponent<RectTransform>();
            hRT.anchorMin = Vector2.zero; hRT.anchorMax = Vector2.one;
            hRT.offsetMin = Vector2.zero; hRT.offsetMax = Vector2.zero;
            _hildirPreviewImg = hildirImgGO.AddComponent<RawImage>();
            _hildirPreviewImg.texture = _hildirPreviewRT;
            _hildirPreviewImg.color = Color.white;
            _hildirPreviewImg.raycastTarget = false;
        }
        ...
        private void RefreshTabPanels()
        {
            ...
            if (_hildirPreviewImg != null) _hildirPreviewImg.gameObject.SetActive(isSell);
        ...
        private void BuildBuyEntries()
        {
            ...
            // Preserve vanilla Hildir trades by reading directly from Trader.m_items.
        ...
        private void SetupHildirPreview()
        {
            ClearHildirPreview();
            if (_currentTrader == null) return;

            const float previewLift = 0.2625f; // ~25% of current 1.05f framing height
            Vector3 spawnPos = HildirSpawnPos + Vector3.up * previewLift;

            ZNetView.m_forceDisableInit = true;
            try { _hildirClone = Instantiate(_currentTrader.gameObject, spawnPos, Quaternion.identity); }
            finally { ZNetView.m_forceDisableInit = false; }

            var rb = _hildirClone.GetComponent<Rigidbody>();
            if (rb != null) Destroy(rb);

            foreach (var mb in _hildirClone.GetComponentsInChildren<MonoBehaviour>(true))
                mb.enabled = false;

            // Force Hildir into a standing pose for the preview.
            var hildirAnim = _hildirClone.GetComponentInChildren<Animator>();
            if (hildirAnim != null)
            {
                SetAnimatorBoolIfPresent(hildirAnim, "Stand", true);
                SetAnimatorBoolIfPresent(hildirAnim, "Sit", false);
                SetAnimatorBoolIfPresent(hildirAnim, "Sitting", false);
                hildirAnim.Update(0.01f);
            }

            // Keep Hildir in the forward-facing preview orientation.
            _hildirClone.transform.rotation = Quaternion.Euler(0f, 0f, 0f);

            int charLayer = LayerMask.NameToLayer("character");
            if (charLayer < 0) charLayer = 9;
            foreach (var t in _hildirClone.GetComponentsInChildren<Transform>(true))
                t.gameObject.layer = charLayer;

            SetupLightRig(ref _hildirLightRig, spawnPos);

            // Fixed camera directly in front of Hildir, framed for standing preview.
            Vector3 center = HildirSpawnPos + Vector3.up * 1.05f;
            _hildirCamGO.transform.position = center + new Vector3(0f, 0.35f, 4.2f);
            _hildirCamGO.transform.LookAt(center);
        }
        ...
        private void ClearHildirPreview()
        {
            if (_hildirLightRig != null) { Destroy(_hildirLightRig); _hildirLightRig = null; }
            if (_hildirClone != null) { Destroy(_hildirClone); _hildirClone = null; }
        }
        ...
        private void EnablePreviewCameras()
        {
            ...
            if (_hildirCam != null) _hildirCam.enabled = false;
        }
        ...
        private void BuildBankPanel()
        {
            ...
            // ── Title ──
            var title = CreateBankText(p, "BankTitle", "Hildir's Bank", 28f, GoldTextColor);
